...
void onTick(CBlob@ this)
{
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		{
			Vec2f aim = this.get_Vec2f("aimPoint");
			Vec2f aimedDirection = aim - pos;

			if (aimedDirection.Length() < 4.0f) this.server_Die();
			...
			if (dif >= 0.0f)
			{
				dif = Maths::Min(dif, 5.0f);
			}
			else
			{
				dif = Maths::Max(dif, -5.0f);
			}
...
void onDie(CBlob@ this)
{
	...
	if (getNet().isServer()) // server
	{
		for (int i = 0; i < 10; i++)
		{
			CBlob@ bullet = server_CreateBlobNoInit("robotclusterbomb");
			if (bullet !is null)
			{
