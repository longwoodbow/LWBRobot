...
void onInit(CBlob@ this)
{
	...
	// 10 seconds of floating around - gets cut down for fire arrow
	// in ArrowHitMap
	...
	this.addCommandID("aimpos sync callback");
}
...
void onTick(CBlob@ this)
{
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		if (owner !is null)
		{
			...
			{
				if (ownerBlob.isMyPlayer() && this.getTickSinceCreated() % 3 == 0)
				{
				...
				Vec2f aim = this.get_Vec2f("aimpos");
				...
				if (dif >= 0.0f)
				{
					dif = Maths::Min(dif, 10.0f);
				}
				else
				{
					dif = Maths::Max(dif, -10.0f);
				}
		...
		f32 randomInn = -10.0f + (( f32(XORRandom(2048)) / 2048.0f) * 20.0f);
		CParticle@ p = ParticlePixelUnlimited(pos, (-velocity * 4.0f).RotateBy(randomInn,Vec2f(0,0)), color, true);
		if(p !is null)
		{
		    p.collides = false;
...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
    ...
    {
    	Vec2f recieved;
    	if (!params.saferead_Vec2f(recieved)) return;

        if (isServer()) this.set_Vec2f("aimpos", recieved);
        if (!isClient())
        {
			CBitStream reserialized;
			reserialized.write_Vec2f(recieved);
        	this.SendCommand(this.getCommandID("aimpos sync callback"), reserialized);
        }
    }
    else if (cmd == this.getCommandID("aimpos sync callback") && isClient())
    {
    	Vec2f recieved;
    	if (!params.saferead_Vec2f(recieved)) return;

        this.set_Vec2f("aimpos", recieved);
    }
}