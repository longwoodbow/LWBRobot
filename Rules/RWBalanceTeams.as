...
void BalanceAll(CRules@ this, RulesCore@ core, BalanceInfo[]@ infos, int type = SCRAMBLE)
{
	...
	for (u32 i = 0; i < len; i++)
	{
		...
		if (p.getTeamNum() != this.getSpectatorTeamNum())
		{
			...
			core.ChangePlayerTeam(p, this.get_u8("gametype") == 2 ? 0 : tempteam);
		}
	}
}
...
void onNewPlayerJoin(CRules@ this, CPlayer@ player)
{
	...
	{
		core.ChangePlayerTeam(player, this.get_u8("gametype") == 2 ? 0 : getSmallestTeam(core.teams));
	}
}
...
void onPlayerRequestTeamChange(CRules@ this, CPlayer@ player, u8 newTeam)
{
	...
	if (newTeam == 255)
	{
		newTeam = this.get_u8("gametype") == 2 ? 0 : getSmallestTeam(core.teams);
	}
	//if a player changing from team 255 (-1), auto-assign
	if (oldTeam == 255)
	{
		oldTeam = this.get_u8("gametype") == 2 ? 0 : getSmallestTeam(core.teams);
		newTeam = oldTeam;
	}
...
{
	// no balancing on boss battle
	if (this.get_u8("gametype") == 2) return;
