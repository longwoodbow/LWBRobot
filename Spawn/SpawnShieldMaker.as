...
void onTick(CBlob@ this)
{
	...
	for (int i = 0; i < 4; i++)
	{
		CParticle@ p1 = ParticlePixelUnlimited(pos + Vec2f(32.0f, 0.0f).RotateBy(f32(XORRandom(256)) * 360.0f / 256.0f), Vec2f_zero, color, true);
		if(p1 !is null)
		{
		    p1.collides = true;
		...
		CParticle@ p2 = ParticlePixelUnlimited(pos + Vec2f(f32(XORRandom(256)) * 32.0f / 256.0f, 0.0f).RotateBy(f32(XORRandom(256)) * 360.0f / 256.0f), Vec2f(0.1f, 0.0f).RotateBy(f32(XORRandom(256)) * 360.0f / 256.0f), color, true);
		if(p2 !is null)
		{
		    p2.collides = true;
