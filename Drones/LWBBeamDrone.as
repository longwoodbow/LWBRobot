...
void onTick(CBlob@ this)
{
	...
	//else
	{
		...
		HitInfo@[] hitInfos;
		CBlob@ target;
		if (map.getHitInfosFromArc(pos, 0.0f, 360.0f, 250.0f, this, @hitInfos))
		{
			for (int i = 0; i < hitInfos.size(); i++)
			{
				...
				if (target !is null)
				{
					...
					{
						Vec2f velocity = target.getPosition() - this.getPosition();
						velocity.Normalize();

						this.server_Hit(target, target.getPosition(), velocity, 0.5f, LWBRobotHitters::energy_module, true);
